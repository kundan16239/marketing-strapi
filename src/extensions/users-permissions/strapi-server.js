...
const issueRefreshToken = (payload, jwtOptions = {}) => {
    ...
    return jwt.sign(
        _.clone(payload.toJSON ? payload.toJSON() : payload),
        process.env.REFRESH_SECRET, { expiresIn: process.env.REFRESH_TOKEN_EXPIRES }
    );
}
...
module.exports = (plugin) => {
    ...
    plugin.controllers.user['userExist'] = async(ctx) => {
        const { identifier } = ctx.request.body
        if (!identifier) return ctx.badRequest('missing.identifier');
        const userWithSameIdentifier = await strapi
